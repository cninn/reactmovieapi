...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    const response = await fetch(
      `https://api.themoviedb.org/3/list/8275714?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`
    );
  ...
  deleteMovie = async (movie) => {
    ...
    const url = `https://api.themoviedb.org/3/list/8275714/remove_item?media_id=${movie.id}&api_key=${process.env.REACT_APP_API_KEY}&language=en-US`;
    ...
    // fetch isteği yapılıyor
    await fetch(url, options);
  

    // fetch işlemi tamamlandığında json verileri burada kullanılabilir
